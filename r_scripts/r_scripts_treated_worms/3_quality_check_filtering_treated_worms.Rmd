...
fill <- c("#e78ac3","#fc8d62") #"#f4cae4","#fdcdac"
...
ggsave(file = "qc_plots/bmalayi_violin_plot_umis_cond_no_pt.pdf", plot = bmalayi_vlnplot_umi_cond_no_pt, dpi = 300)
ggsave(file = "qc_plots/bmalayi_violin_plot_genes_cond_no_pt.pdf", plot = bmalayi_vlnplot_unique_gene_cond_no_pt, dpi = 300)
...
ggsave(file = "qc_plots/bmalayi.treated_vlnplot_umi_section.pdf", plot = bmalayi.treated_vlnplot_umi_section_no_pt, dpi = 300)
ggsave(file = "qc_plots/bmalayi.treated_vlnplot_unique_gene_section.pdf", plot = bmalayi.treated_vlnplot_unique_gene_section_no_pt, dpi = 300)
...
p1 <- ggplot(worm_sample_raw_unfil_df, aes(T1, T2)) + geom_point(alpha = 1,colour = "#8da0cb") + stat_cor(method = "pearson", label.x = 0, label.y = 3.7) + geom_smooth(method = "lm", fullrange=TRUE, color="#8da0cb", se = TRUE, level = 0.95)  + theme_minimal() + labs(title="Average gene expression between treated samples",
       x="Treated worm T1 (log (1 + avg raw UMI counts per spot))", y = "Treated worm T2 (log (1 + avg raw UMI counts per spot))")
...
colnames(worm_section_raw_unfil_df) <- c("T1_1","T1_3","T1_4","T1_5","T2_1")
worm_section_raw_unfil_df
...
for (i in colnames(worm_section_raw_unfil_df)) {
  ...
  for (j in colnames(worm_section_raw_unfil_df)){ 
    ...
    p <- ggplot(worm_section_raw_unfil_df, aes_(as.name(first_section),as.name(second_section))) + geom_point(alpha = 1,colour = "#8da0cb") + stat_cor(method = "pearson", label.x = 0, label.y = 3.7) + geom_smooth(method = "lm", fullrange=TRUE, color="#8da0cb", se = TRUE, level = 0.95)  + theme_minimal() + labs(title="Average gene expression between treated samples",           x=paste0("Treated worm ", first_section, " (log (1 + avg raw UMI counts per spot))"), y = paste0("Treated worm", second_section, " (log (1 + avg raw UMI counts per spot))"))
    
    ggsave(file = paste0("qc_plots/bmalayi.treated_corr_worm_raw_unfilt_section_", first_section, "_", second_section, ".pdf"), plot = p, dpi = 300)
  }
}
...
ggsave(file = "qc_plots/wBm_vlnplot_unique_gene_cond_no_pt .pdf", plot = wBm_vlnplot_unique_gene_cond_no_pt , dpi = 300)
ggsave(file = "qc_plots/wBm_vlnplot_umi_cond_no_pt.pdf", plot = wBm_vlnplot_umi_cond_no_pt, dpi = 300)
...
ctrl_spots.spot.filt <- SubsetSTData(ctrl_spots_filt, expression = nFeature_RNA >= 30 & nCount_RNA >= 50 & percent.mito < 3 & percent.ribo < 30 & nCount_RNA < 10000) #& percent.mito < 10 & percent.ribo < 40
